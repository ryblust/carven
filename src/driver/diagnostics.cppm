...
auto report_errors(std::span<const ParseError> errors, std::string_view source, std::string_view filepath) noexcept -> void {
    if (errors.empty()) {
        return;
    }
    ...
    for (auto i = 0uz; i < errors.size(); ++i) {
        ...
        for (auto col = 0uz; col < column; ++col) {
            std::print(" ");
        }

        for (auto pos = 0uz; pos < span_len && column + pos < line_end - line_start; ++pos) {
            std::print("^");
        }
