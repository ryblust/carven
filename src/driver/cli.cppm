...
export auto __zero_main__(int argc, const char** argv) noexcept -> int {
    ...
    if (argc > 2) {
        const auto arg = std::string_view(argv[2]);
        if (arg == "--help" || arg == "-h") return render_command_help(*command);
    }

    auto driver = Driver();
    if (!driver.parse_flags({argv + 2, argv + argc - 2})) return 1;
