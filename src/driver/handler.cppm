...
auto dump_stmt(const Stmt& stmt, std::string_view source, std::string& out, int level) noexcept -> void {
    std::visit(Overloaded {
        ...
        [&](const WhileStmt& s) -> void {
            ...
            out.append(") ");
            if (std::holds_alternative<BlockStmt>(*s.body)) {
                out.append("{\n");
                for (const auto* st : std::get<BlockStmt>(*s.body).statements) dump_stmt(*st, source, out, level + 1);
                dump_indent(out, level);
                out.append("}\n");
            } else {
                out.append("\n");
...
export auto dump_ast(const ParseResult& result, std::string_view source) noexcept -> std::string {
    auto out = std::string();
