...
Lexer::Lexer(SourceView source_view) noexcept
...
auto Lexer::run() noexcept -> Diagnosed<TokenBuffer> {
    while (true) {
...
auto Lexer::at_end(std::size_t lookahead) const noexcept -> bool {
    return static_cast<std::size_t>(position) + lookahead >= source.size();
}

auto Lexer::current(std::size_t lookahead) const noexcept -> char {
...
auto Lexer::span() const noexcept -> Span {
    return Span::from_bounds(token_start, position);
}

auto Lexer::append_token(TokenKind kind) noexcept -> void {
    result.value.append_token(kind, span());
    previous_token_kind = kind;
}

auto Lexer::append_literal_token(TokenLiteralValue value) noexcept -> void {
    result.value.append_literal_token(span(), std::move(value));
    previous_token_kind = result.value.tokens().back().kind;
}

auto Lexer::diagnose(std::string_view message, Span diagnostic_span) noexcept -> void {
    result.diagnostics.push_back(DiagnosticBuilder(DiagnosticCode::Lexical, std::string(message))
                                     .primary(locate(source_id, diagnostic_span))
                                     .build());
}

auto Lexer::diagnose_invalid(std::string_view message) noexcept -> void {
    append_token(TokenKind::Invalid);
    diagnose(message, span());
}

auto Lexer::consume_utf8() noexcept -> bool {
...
auto Lexer::skip_whitespace_and_comments() noexcept -> void {
    while (!at_end()) {
...
auto Lexer::scan_interpolation_expression() noexcept -> void {
...
auto Lexer::scan_token() noexcept -> void {
    token_start = position;
    ...
    switch (value) {
        ...
        case '>':
            ...
            return;
        case '&':
            append_token(match('&') ? AmpersandAmpersand : match('=') ? AmpersandEqual : Ampersand);
            return;
...
auto Lexer::scan_identifier() noexcept -> void {
...
auto Lexer::scan_number() noexcept -> void {
...
auto Lexer::scan_string(bool c_string) noexcept -> void {
    ...
    if (scanned.has_value()) {
        if (c_string) {
            append_literal_token(CStringLiteralValue {.bytes = std::move(scanned->value.bytes)});
        } else {
        ...
        const auto end =
            std::min(position, start + static_cast<std::uint32_t>(scanned.error().error_length));
        diagnose(
...
auto Lexer::scan_character() noexcept -> void {
...
auto Lexer::consume_line_ending() noexcept -> bool {
    if (match('\n')) {
        return true;
    }
    if (!match('\r')) {
...
auto Lexer::scan_cpp_source_fragment() noexcept -> void {
    ...
    while (!at_end()) {
        ...
        if (closing_size == fence_size && (at_end() || current() == '\n' || current() == '\r')) {
