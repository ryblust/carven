module carven:semantic.analysis.elaboration.expressions.callable.impl;
...
auto build_expression(
    ...
    if (callable_result.has_value()) {
        ...
        const auto common_count = std::min(arguments.size(), parameter_types.size());
        for (auto index = 0uz; index < common_count; ++index) {
            if (!compatible(
                    module_analysis,
                    parameter_types[index].type,
                    expression_type(module_analysis, arguments[index].expression)
                )) {
                module_analysis.emit(
            ...
            if (parameter_types[index].access != arguments[index].access) {
                module_analysis.emit(
                    ast.expression(call.arguments[index].expression).span,
            ...
            if (arguments[index].access == HIRAccessMode::Write) {
...
auto build_expression(
    ...
    const auto callable = builder.append_callable(
        ...
        result,
        contract_failures,
        lambda.throw_clause.has_value() ? HIRFailureContractKind::Declared
