module carven:semantic.analysis.body.aggregate.impl;
...
auto BodyElaborator::array_expression(
    ...
    for (const auto element_id : array.element_ids) {
        ...
        auto value = consume_value(*element, ast.expression(element_id).span, AccessMode::Read);
        if (!value.has_value()) {
        ...
        if (const auto* values =
            ...
            for (auto value_id : values->values) {
                ...
                if (access == AccessMode::Write) {
                    auto place = consume_place(*value, ast.expression(value_id).span);
                    if (!place.has_value()) {
                        return std::unexpected(place.error());
                    }
                    operands.push_back(
                        {.access = access, .expression = std::move(place->expression)}
                    );
                } else {
                    auto read = consume_value(*value, ast.expression(value_id).span, access);
                    if (!read.has_value()) {
                        return std::unexpected(read.error());
                    }
                    operands.push_back({.access = access, .expression = std::move(*read)});
                }
            }
