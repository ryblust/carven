module carven:semantic.analysis.body.expression_site.impl;
...
BodyExpressionSite::BodyExpressionSite(BodyElaborator& body, bool allow_pointer_narrowing) noexcept
    : body(body),
      allow_pointer_narrowing(allow_pointer_narrowing) {}

auto BodyExpressionSite::draft() noexcept -> ProgramDraft& {
    return body.draft();
}

auto BodyExpressionSite::syntax() const noexcept -> ASTView {
    return body.ast;
}

auto BodyExpressionSite::fail(Span span, DiagnosticCode code, std::string message) noexcept
    -> AnalysisFailure {
    return body.fail(span, code, std::move(message));
}

auto BodyExpressionSite::read(ASTExprID id, std::optional<ConstructionTypeRef> expected) noexcept
    -> AnalysisResult<Value> {
    return body.expression(id, expected, allow_pointer_narrowing);
}

auto BodyExpressionSite::present(const Value&) const noexcept -> bool {
    return true;
}

auto BodyExpressionSite::unavailable() const noexcept -> Value {
    invariant_violation("body expression became unavailable");
}

auto BodyExpressionSite::type(const Value& value) const noexcept -> ConstructionTypeRef {
    return value.type();
}

auto BodyExpressionSite::known(const Value& value) const noexcept -> std::optional<ConstantID> {
    return value.constant();
}

auto BodyExpressionSite::external(ConstructionTypeRef type) const noexcept -> bool {
    return body.is_cpp_type(type);
}

auto BodyExpressionSite::dereference(const ASTPrefixExpr& source, Span span) noexcept
    -> AnalysisResult<Value> {
    return body.dereference_expression(source, span);
}

auto BodyExpressionSite::supports_equality(ConstructionTypeRef type) noexcept -> bool {
    return type_supports_equality(draft(), type);
}

auto BodyExpressionSite::numeric_enum(ConstructionTypeRef type) noexcept -> bool {
...
auto BodyExpressionSite::resolve_type(ASTTypeID type) noexcept
    -> AnalysisResult<ConstructionTypeRef> {
    return body.resolve_type(type);
}

auto BodyExpressionSite::c_string(std::string_view bytes, Span span) noexcept -> Value {
    const auto type =
...
auto BodyExpressionSite::constant(ConstantID constant, Span span) noexcept -> Value {
    return body
        .make_built(draft().constant_copy(constant).type, SemConstant {.constant = constant}, span);
}

auto BodyExpressionSite::enter_operand_execution(bool executed) noexcept -> BodyReferencePathGuard {
    return BodyReferencePathGuard(body.reference_path_reachable, executed);
}

auto BodyExpressionSite::finish_unary(
...
auto BodyExpressionSite::finish_binary(
...
auto BodyExpressionSite::finish_cast(
    CastKind kind,
...
auto BodyExpressionSite::finish_short_circuit(
    bool conjunction,
    Value left,
    Value right,
    std::optional<ConstantID> known,
...
auto BodyExpressionSite::external_unary(UnaryOperator operation, Value operand, Span span) noexcept
    -> AnalysisResult<Value> {
...
auto BodyExpressionSite::external_cast(ConstructionTypeRef type, Value operand, Span span) noexcept
    -> AnalysisResult<Value> {
    return external_operation(
...
auto BodyExpressionSite::external_binary(
    BinaryOperator operation,
    Value left,
    Value right,
    Span span
...
auto BodyExpressionSite::extension(
...
auto BodyExpressionSite::resolve_name(std::string_view name, Span span) noexcept
    -> AnalysisResult<ResolvedConstantName> {
    return body.resolve_constant_name(name, span);
}

auto BodyExpressionSite::resolve_enum_qualifier(ASTExprID id) noexcept
    -> AnalysisResult<std::optional<TypeID>> {
    return body.resolve_enum_qualifier(id);
}

auto BodyExpressionSite::resolve_enum_case(TypeID type, std::string_view name, Span span) noexcept
    -> AnalysisResult<ResolvedEnumCase> {
    return body.resolve_constant_enum_case(type, name, span);
}

auto BodyExpressionSite::is_numeric_enum(TypeID type) noexcept -> bool {
    return numeric_enum(type);
}

auto BodyExpressionSite::admits(const ASTExpr&) const noexcept -> bool {
    return true;
}

auto BodyExpressionSite::spelling(Span span) const noexcept -> std::string {
    return body.spelling(span);
}

auto BodyExpressionSite::invalid_enum_qualifier(Span span) noexcept -> AnalysisResult<Value> {
...
auto BodyExpressionSite::convert_argument(
    Value& value,
    ConstructionTypeRef type,
    Span span
...
auto BodyExpressionSite::enum_constructor(
...
auto BodyExpressionSite::finish_enum_case(
    TypeID type,
    EnumCaseID selected,
...
auto BodyExpressionSite::extension(
    ...
    const auto append = [&](this const auto& self,
        ...
        for (const auto& part : parts) {
            ...
            auto built = body.build_call_argument(hole->expression, AccessMode::Read, std::nullopt);
            if (!built) {
...
auto BodyExpressionSite::finish_text(
...
auto BodyExpressionSite::finish_slice_call(
    ...
    if (intrinsic == SliceIntrinsic::FromArray) {
        if (const auto* id = std::get_if<TypeID>(&receiver_type)) {
            ...
            result_type = draft().intern_type({.value = SliceTypeValue {.element = array.element}});
        } else {
...
auto BodyExpressionSite::finish_text_call(
    TextIntrinsic intrinsic,
    ...
    const auto argument_type = intrinsic == TextIntrinsic::FromUTF8Unchecked
        ? draft().intern_type(
              {.value = SliceTypeValue {.element = draft().intern_builtin_type(BuiltinType::U8)}}
          )
        : draft().intern_builtin_type(
              intrinsic == TextIntrinsic::FromU32Unchecked ? BuiltinType::U32
                  : intrinsic == TextIntrinsic::Push       ? BuiltinType::Char
                                                           : BuiltinType::Str
          );
    for (const auto& argument : arguments) {
        auto built = body.build_call_argument(argument.expression, AccessMode::Read, argument_type);
        if (!built) {
            return std::unexpected(built.error());
        }
        append_pending_failures(pending, built->pending_failures);
...
auto BodyExpressionSite::member(const ASTMemberExpr& source, Value operand, Span span) noexcept
    -> AnalysisResult<Result> {
    return body.select_member(source, span, std::move(operand));
}

auto BodyExpressionSite::member_call(
...
auto BodyExpressionSite::call(const ASTCallExpr& source, Span span) noexcept
    -> AnalysisResult<Value> {
    return body.call_expression(source, span);
}

auto BodyExpressionSite::finish(
    ConstructionTypeRef type,
...
auto BodyExpressionSite::external_operation(
