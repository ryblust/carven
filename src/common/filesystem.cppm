...
auto map_file(std::string_view path) noexcept -> std::optional<std::pair<void*, std::size_t>> {
#if defined(_WIN32)
...
auto unmap_file(void* data, std::size_t size) noexcept -> void {
#if defined(_WIN32)
    (void)size;
    if (data) {
...
auto write_file(const std::filesystem::path& path, std::string_view content) noexcept -> bool {
