...
auto TargetPlanConstruction::allocate_names() noexcept -> void {
    const auto allocator = TargetNameAllocator {};
    auto implementation_logical_paths = std::vector<std::string> {};
    implementation_logical_paths.reserve(semantic.modules().size());
    for (auto index = 0uz; index < semantic.modules().size(); ++index) {
        ...
        implementation_logical_paths.push_back(
            module_implementation_logical_path(path.components())
        );
    }
    ...
    auto modules = std::vector<TargetModuleGenerationPlan> {};
    ...
    for (auto index = 0uz; index < semantic.modules().size(); ++index) {
        ...
        modules.push_back({
            ...
            .implementation_logical_path = std::move(implementation_logical_paths[index]),
    ...
    for (const auto& symbol : semantic.symbols()) {
        if (symbol.module_id.has_value() || !symbol.place.has_value()) {
            continue;
        }
        ...
        const auto scope = semantic.place(*symbol.place).scope;
        scoped_source_names[scope.index()].insert(std::string(reserved.spelling()));
    }

    names = TargetNamePlan {
