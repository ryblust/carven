...
public:
    TargetReferenceWalker(
        ...
        std::span<const TargetModuleGenerationPlan> modules
    ) noexcept
        : semantic(semantic),
          modules(modules),
          result {
              .surface = decltype(result.surface)(modules.size()),
              .surface_declarations = decltype(result.surface_declarations)(modules.size()),
              .implementation = decltype(result.implementation)(modules.size()),
          } {}

    auto collect() noexcept -> TargetReferenceAnalysis {
        for (auto index = 0uz; index < modules.size(); ++index) {
            const auto module_id = ProgramModuleID::from_index(static_cast<std::uint32_t>(index));
            for (const auto declaration : modules[index].surface_declarations) {
...
private:
    ...
    auto collect_callable(
        ...
        for (const auto failure : semantic.failure_set(callable.failure_set).members) {
        ...
        std::visit(
            Overloaded {
                ...
                [&](const HIRMatchExpr& value) noexcept {
                    collect_expression(module_id, value.subject);
                    for (const auto& arm : value.arms) {
                        collect_match_arm(module_id, arm);
                    }
                },
                [&](const HIRTryExpr& value) noexcept {
...
auto TargetPlanConstruction::plan_references_and_interfaces() noexcept -> void {
    if (!names.has_value()) {
        invariant_violation("target references require completed target names");
    }
    auto& modules = names->modules;
    for (const auto nominal : semantic.nominal_storage_order()) {
        ...
        if (visibility(semantic, declaration) != DeclarationVisibility::Module) {
            modules[module_id.index()].surface_declarations.push_back(declaration);
        } else {
            modules[module_id.index()].implementation_nominal_order.push_back(nominal);
        }
    }
    for (auto index = 0uz; index < semantic.modules().size(); ++index) {
        ...
        for (const auto& item : semantic.hir_module(module_id).items) {
            ...
            if (function != nullptr
                ...
                modules[index].surface_declarations.push_back(HIRDeclarationRef {*function});
            }
        }
    }

    auto references = TargetReferenceWalker(semantic, modules).collect();
    ...
    for (auto index = 0uz; index < modules.size(); ++index) {
        if (!modules[index].surface_declarations.empty()) {
    ...
    auto nominal_order = std::flat_map<HIRNominalDeclRef, std::size_t>();
    for (auto index = 0uz; index < semantic.nominal_storage_order().size(); ++index) {
        nominal_order.emplace(semantic.nominal_storage_order()[index], index);
    }

    interface_components.reserve(component_members.size());
    for (auto component_index = 0uz; component_index < component_members.size();
         ++component_index) {
            ...
            for (const auto& declaration : references.surface_declarations[member.index()]) {
                ...
                for (const auto& [nominal, requirement] : declaration.requirements) {
                    ...
                    if (!module_component[owner.index()].has_value()
                        ...
                        || nominal_order.at(nominal) <= nominal_order.at(*current)) {
                        continue;
                    }
                    if (requirement == TargetTypeCompleteness::CompleteDefinition) {
                        invariant_violation(
                            "nominal definition order does not satisfy a complete dependency"
                        );
                    }
        ...
        std::ranges::sort(
            ...
            [&](HIRNominalDeclRef left, HIRNominalDeclRef right) noexcept {
                ...
                const auto left_path = module_path(left_module);
                const auto right_path = module_path(right_module);
                if (left_path != right_path) {
                    return left_path < right_path;
                }
                return nominal_order.at(left) < nominal_order.at(right);
            }
        );
        auto forward_declarations = std::vector<TargetHeaderForwardDeclarationPlan>();
        forward_declarations.reserve(ordered_forward_nominals.size());
        for (const auto nominal : ordered_forward_nominals) {
            forward_declarations.push_back({
                .module_id = owner_module(semantic, declaration_ref(nominal)),
                .declaration = nominal,
            });
        }
        auto declarations = std::vector<TargetHeaderDeclarationPlan>();
        for (const auto nominal : semantic.nominal_storage_order()) {
            const auto declaration = declaration_ref(nominal);
        ...
        auto prerequisite_header_paths = std::vector<std::string>();
        prerequisite_header_paths.reserve(component_predecessors[component_index].size());
        for (const auto predecessor : component_predecessors[component_index]) {
            if (predecessor >= interface_components.size()) {
                invariant_violation("interface prerequisite was not built dependency-first");
            }
            prerequisite_header_paths.push_back(interface_components[predecessor].logical_path);
        }
        const auto& anchor_path = semantic.provenance().module_record(members.front()).path;
        interface_components.push_back({
            .logical_path = interface_component_logical_path(anchor_path.components()),
            .prerequisite_header_paths = std::move(prerequisite_header_paths),
    ...
    for (auto index = 0uz; index < modules.size(); ++index) {
        auto& module_plan = modules[index];
        ...
        module_plan.interface_header_paths.reserve(used_components.size());
        for (const auto component : used_components) {
            module_plan.interface_header_paths.push_back(
                interface_components[component].logical_path
            );
        }
    }
}
