...
public:
    ...
    ~ExpressionBuilder() noexcept { owner.active_frame = previous_frame; }
    ...
    auto initialize(
        ...
        auto recipe =
            build(initialization.initializer, nullptr, true, ConstructionUse::Consume, true);
        if (!statements.continues()) {
            ...
            if (assignment.compound && !external) {
                ...
                switch (*assignment.compound) {
                    ...
                    case BinaryOperator::Divide:
                        operation = TargetAssignmentOperator::Divide;
                        break;
...
private:
    ...
    auto build(
        ...
        if (const auto* logic = std::get_if<ConstructionShortCircuit>(&value.value)) {
            ...
            branches.push_back(
                {.condition = std::move(test), .body = std::move(statements).finish()}
            );
        ...
        auto needs_stable_source = std::holds_alternative<SemArrayAdopt>(value.operation.value);
        if (std::holds_alternative<SemBorrowCallable>(value.operation.value)) {
            const auto& source_type =
                owner.context.semantic().types().type(source(recipe.operands.front()).type).value;
            if (const auto* closure = std::get_if<ClosureTypeValue>(&source_type)) {
                const auto body =
                    owner.context.semantic().declarations().body_for_callable(closure->callable);
                needs_stable_source =
                    !owner.context.semantic().bodies().body(*body).inputs().captures.empty();
            }
            // Functions and stateless closures adapt by value. Only a captured
            // closure supplies a borrowed object whose storage must be stable.
        }
    ...
    auto raw(
        ...
        if (saved(recipe)) {
            ...
            if (saved(recipe)->kind == SavedKind::Success) {
                result =
                    member_expression(std::move(result), TargetIdentifier::from_spelling("value"));
            }
            return result;
        }
    ...
    auto anchor(
        ...
        if (std::holds_alternative<SemCppCall>(value.operation.value)
            ...
            const auto* query =
                native == nullptr ? nullptr : std::get_if<CppQueryType>(&native->form);
            if (query == nullptr) {
        ...
        const auto place = value.category == SemanticValueCategory::Place
            && (use == ConstructionUse::Place || use == ConstructionUse::ConstPlace);
    ...
    auto complete_call(
        ...
        const auto signature = std::visit(
            Overloaded {
                ...
                [](const auto&) static noexcept -> CallableSignatureID {
                    invariant_violation("fallible call has no signature");
                }
            },
            callee
        );
        ...
        const auto storage = LoweringDeferredStorage {
            .name = outcome,
            .value_type = owner.context.outcome_type(signature)
        };
...
auto BodyRealizer::operand(ConstructionOperand source, RealizationLiteralContext literal) noexcept
    -> Lowered<TargetExpr> {
    auto result = ExpressionBuilder(*this, source.expression)
