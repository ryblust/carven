module carven:backend.emission.item.impl;
    ...
    return value.visit(
        Overloaded {
            ...
            [&](const TargetVariableDecl& variable) noexcept {
                ...
                if (variable.constexpr_specifier) {
                    prefix += "constexpr ";
                }
                const auto left = concat({
                    ...
                    text(" "),
                    render_identifier(variable.name),
                });
                const auto right = concat({text("= "), render_expression(variable.initializer)});
                return concat({
                    choice({
