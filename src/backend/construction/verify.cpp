...
ConstructionVerifier::ConstructionVerifier(
    const BodyConstruction& body,
    const SemIRBody& metadata
) noexcept
    : body(body),
...
auto ConstructionVerifier::finish() noexcept -> std::expected<void, ConstructionViolation> {
...
void ConstructionVerifier::fail(
    ConstructionViolationKind kind,
    std::optional<ProgramOriginID> origin,
    std::string_view message
) noexcept {
...
auto ConstructionVerifier::enter(Visit& state, std::optional<ProgramOriginID> origin) noexcept
    -> bool {
...
void ConstructionVerifier::lifetime(LifetimeRegionID id, ProgramOriginID origin) noexcept {
...
void ConstructionVerifier::pattern(PatternID pattern_id, ProgramOriginID origin) noexcept {
...
void ConstructionVerifier::failure(
    const ConstructionFailureExit& destination,
    const Control& control,
    ProgramOriginID origin
) noexcept {
    ...
    if (handler == nullptr ? control.handler.has_value() : control.handler != handler->handler) {
        fail(
            ConstructionViolationKind::InvalidControl,
            origin,
...
void ConstructionVerifier::expression(
    ConstructionExpressionID id,
    const Control& control,
    std::optional<ProgramOriginID> parent
) noexcept {
    ...
    std::visit(
        Overloaded {
            ...
            [&](const ConstructionTestReport& value) noexcept {
                ...
                if (value.message) {
                    expression(*value.message, control, source.origin);
                }
            },
...
void ConstructionVerifier::statement(
    const ConstructionStatement& source,
    ConstructionRegionID owner,
    const Control& control
) noexcept {
        ...
        Overloaded {
            ...
            [&](const ConstructionLoopTransfer& value) noexcept {
                ...
                const auto is_loop = target.statements.size() == 1uz
                    && !target.result
                    && (std::holds_alternative<ConstructionLoop>(target.statements.front().value)
...
void ConstructionVerifier::loop_owner(ConstructionRegionID id, ProgramOriginID origin) noexcept {
...
void ConstructionVerifier::region(
    ConstructionRegionID id,
    const Control& control,
    std::optional<ProgramOriginID> parent
) noexcept {
