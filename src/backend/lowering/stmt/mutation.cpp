module carven:backend.lowering.stmt.mutation.impl;
...
auto lower_statement(
    ...
    if (statement.op != HIRAssignmentOperator::Assign
        ...
        && is_integer_type(context, context.source().expression(statement.target).type)) {
            ...
            TargetExprStmt {
                .expression = context.target().append_expression({
                    .value = TargetCallExpr {
                        ...
                        .template_argument_type_ids = {lower_type(
                            context,
                            context.source().expression(statement.target).type
                        )},
...
auto lower_statement(
    ...
    const auto target_value = target.expression;
    auto statements = std::move(target.prelude);
    if (is_integer_type(context, context.source().expression(statement.target).type)) {
        return with_prelude(
            context,
            std::move(statements),
            TargetExprStmt {
