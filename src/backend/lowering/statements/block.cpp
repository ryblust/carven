module carven:backend.lowering.statements.block.impl;
...
auto lower_block(
    ...
    const auto& block = context.semantic().block(id);
    ...
    if (block.result.has_value()) {
        ...
        const auto returned = control.test_exit.has_value() && control.test_exit->result.has_value()
            ? test_success_expression(context, *control.test_exit->result, expression.expression)
            : expression.expression;
        auto result_statement = return_result
                                          ? TargetStmtValue {
                                                TargetReturnStmt {.expression = returned},
                                            }
                                          : TargetStmtValue {
...
auto lower_outcome_block(
    ...
    const auto matches_active_carrier = control.failure.has_value()
        && control.failure->carrier.result == natural_carrier.result
        && control.failure->carrier.failure_set == natural_carrier.failure_set;
