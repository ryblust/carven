module carven:backend.lowering.statements.binding.impl;
...
auto lower_variable_binding(
    ...
    const auto& symbol = context.semantic().symbol(named_target->symbol);
    if (symbol.place.has_value() && context.requires_mutable_value_binding(*symbol.place)) {
        return TargetVariableBinding::MutableValue;
    }
    return TargetVariableBinding::ConstValue;
}
...
auto lower_statement(
    ...
    const auto* attempt =
        std::get_if<HIRTryExpr>(&context.semantic().expression(statement.expression).value);
    if (attempt != nullptr
        && is_void_type(context, context.semantic().expression(statement.expression).type)) {
        ...
        TargetVariableStmt {
            ...
            .name = named_target == nullptr
                ...
                : binding_identifier(
                      context,
                      named_target->symbol,
                      context.semantic().expression_facts(statement.initializer).evaluation_effect
                  ),
            .type = target_type,
