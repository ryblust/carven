module carven:backend.lowering.expr.constant.impl;
...
auto lower_constant_value(TargetCallableLowerer& context, HIRConstantID constant) noexcept
    -> TargetExprID {
    const auto& fact = context.source().constant(constant);
    return std::visit(
        [&](const auto& value) noexcept -> TargetExprID {
            ...
            if constexpr (std::same_as<Value, HIRIntegerConstant>) {
                ...
                return context.target().append_expression({
                    .value = TargetLiteralExpr {
                        .value = TargetStringLiteral {
                            .bytes =
                                std::string(context.source().provenance().spelling(value.value)),
                ...
                if (const auto* builtin =
                        std::get_if<HIRBuiltinTypeValue>(&context.source().type(fact.type).value);
                ...
                return name_expression(
                    ...
                    symbol_reference_name(
                        context,
                        context.source().enum_case(value.enum_case).symbol
                    )
                );
