module carven:backend.lowering.expressions.handlers.impl;
...
auto alternative_patterns(
    TargetCallableLowerer& context,
    TargetExprID carrier_expression,
    ...
    const auto subject =
        failure.has_value() && pattern_requires_subject(context, *alternative.inner)
        ...
        : carrier_expression;
    return lower_pattern(context, subject, *alternative.inner);
}
...
auto handler_body(
    ...
    if (!failure_set.has_value() || context.failure_set(*failure_set).ordered_members.empty()) {
...
auto lower_catch_handlers(
    ...
    if (exclusive) {
        ...
        for (auto arm_index = 0uz; arm_index < arms.size(); ++arm_index) {
            ...
            if (context.failure_set(facts[arm_index].accepted_failure_set)
            ...
            for (auto alternative_index = 0uz; alternative_index < arm.alternatives.size();
                ...
                const auto type_condition = failure.transform([&](TargetTypeID type) noexcept {
                ...
                auto patterns =
                    alternative_patterns(context, carrier_expression, alternative, failure);
                for (auto& pattern : patterns) {
                    auto condition = type_condition;
    ...
    auto matched = std::optional<TargetExprID> {};
    ...
    if (!expression_handlers) {
        ...
        matched = name_expression(context, TargetName {*matched_name});
    ...
    for (auto arm_index = 0uz; arm_index < arms.size(); ++arm_index) {
        ...
        for (auto alternative_index = 0uz; alternative_index < arm.alternatives.size();
            ...
            auto patterns = alternative_patterns(context, carrier_expression, alternative, failure);
            for (auto& pattern : patterns) {
