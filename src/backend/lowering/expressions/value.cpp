module carven:backend.lowering.expressions.value.impl;
        ...
        for (auto later = index + 1; later < lowered.size(); ++later) {
            ...
            if ((reordered || !lowered[later].prelude.empty())
                && !TargetEvaluationSequencer::effects_commute(
                    context.semantic().expression_facts(expressions[index]).evaluation_effect,
                    context.semantic().expression_facts(expressions[later]).evaluation_effect
                )) {
...
auto lower_expression(
    ...
    if (const auto constant = context.semantic().expression(id).constant) {
...
auto lower_expression(
    ...
    const auto& array_type =
        std::get<HIRArrayTypeValue>(context.semantic().type(source.type).value);
...
auto lower_expression(
    ...
    const auto& source = context.semantic().expression(id);
    const auto* nominal =
        std::get_if<HIRStructTypeValue>(&context.semantic().type(source.type).value);
    if (nominal == nullptr) {
        invariant_violation("HIR structure construction has a non-structure result type");
    }
    const auto& structure = context.semantic().structure(nominal->structure);
    const auto enclosing = context.semantic().provenance().spelling(structure.name);
    ...
    for (auto index = 0uz; index < expression.fields.size(); ++index) {
        const auto& field = expression.fields[index];
        ...
        values.push_back({
            ...
            TargetFieldInitializer {
                .name = context.name_allocator().source(
                    context.semantic().provenance().spelling(
                        structure.fields[declaration_index].name
                    ),
                    enclosing
                ),
                .value = lowered.values[index],
            },
        });
    }
...
auto lower_expression(
    ...
    const auto callee = name_expression(
        context,
        symbol_reference_name(context, context.semantic().enum_case(expression.enum_case).symbol)
    );
