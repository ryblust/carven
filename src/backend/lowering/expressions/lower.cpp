module carven:backend.lowering.expressions.lower.impl;
...
auto lower_expression(
    ...
    const auto raw = context.target().append_expression({
        .value = TargetRawFragment {
            .bytes = std::string(context.semantic().provenance().spelling(expression.bytes)),
        },
    });
    const auto* builtin = std::get_if<HIRBuiltinTypeValue>(
        &context.semantic().type(context.semantic().expression(id).type).value
    );
    if (builtin == nullptr
...
auto lower_expression(
    ...
    const auto& source = context.semantic().expression(id);
    const auto& facts = context.semantic().expression_facts(id);
    const auto failure_set = carrier_failure_set(context, id);
    const auto effectful = !context.failure_set(failure_set).ordered_members.empty();
    const auto exits_test = facts.exits_test;
...
auto lower_expression(
    ...
    if (!effectful) {
        auto body = lower_value_match(context, expression.subject, expression.arms, match_control);
...
auto lower_expression(
    ...
    const auto& source = context.semantic().expression(id);
    const auto& body_source = context.semantic().block(expression.body);
    const auto& facts = context.semantic().expression_facts(id);
    const auto& body_facts = context.semantic().block_facts(expression.body);
    const auto& attempt_facts = *facts.attempt;
    const auto exits_test = facts.exits_test;
    if (!body_source.result.has_value() && !is_void_type(context, source.type)) {
        std::unreachable();
    }
    const auto protected_failure_set = body_facts.outward_failure_set;
    ...
    const auto effectful = !context.failure_set(output_failure_set).ordered_members.empty();
    const auto target_result = lower_type(context, source.type);
    const auto carrier = effectful
    ...
    if (!protected_value.has_value()) {
        ...
        protected_value = body_facts.exits_test
