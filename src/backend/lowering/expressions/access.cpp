module carven:backend.lowering.expressions.access.impl;
...
auto requires_operand_sequencing(
    ...
    return !lowered_index.prelude.empty()
        || !TargetEvaluationSequencer::effects_commute(
            context.semantic().expression_facts(operand).evaluation_effect,
            context.semantic().expression_facts(index).evaluation_effect
        );
}
...
auto lower_mutation_expression(
    ...
    if (std::holds_alternative<HIRArrayTypeValue>(
            context.semantic().type(context.semantic().expression(index.operand_id).type).value
        )) {
...
auto lower_expression(
        ...
        operand = TargetEvaluationSequencer::materialize(
            ...
            operand_foreign ? MaterializationKind::Preserve
                            : TargetEvaluationSequencer::read_materialization(
                                  context,
                                  context.semantic().expression(expression.operand_id).type
                              ),
            TargetMaterializationReason::EvaluationOrder
        );
    }
    ...
    const auto selected =
        std::holds_alternative<HIRArrayTypeValue>(
            context.semantic().type(context.semantic().expression(expression.operand_id).type).value
        )
        ? call_expression(
              context,
              name_expression(context, TargetSymbol::RuntimeCheckedArrayIndex),
...
auto lower_expression(
    ...
    if (uses_scope_access(expression)) {
        const auto& operand = context.semantic().expression(expression.operand_id);
                ...
                .expression = context.target().append_expression({
                    .value = TargetScopeMemberExpr {
                        .operand =
                            TargetRawFragment {
                                .bytes = std::string(
                                    context.semantic().provenance().spelling(raw->bytes)
                                ),
                            },
