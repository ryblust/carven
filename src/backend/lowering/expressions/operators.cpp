module carven:backend.lowering.expressions.operators.impl;
...
auto lower_expression(
    ...
    while (true) {
        const auto* cast = std::get_if<HIRCastExpr>(&context.semantic().expression(owner).value);
        if (cast == nullptr || cast->kind != HIRCastKind::Identity) {
            break;
        }
        owner = cast->operand_id;
    }
    if (!std::holds_alternative<HIRNameExpr>(context.semantic().expression(owner).value)) {
        return operand;
    }
    return {
...
auto lower_expression(
    ...
    const auto& source = context.semantic().expression(id);
    auto operand = lower_expression(context, expression.operand_id, control);
    const auto operand_foreign =
        is_foreign_type(context, context.semantic().expression(expression.operand_id).type);
    ...
    if (expression.op == HIRUnaryExpr::Operator::Negate
        && is_integer_type(context, context.semantic().expression(expression.operand_id).type)) {
        return {
            .prelude = std::move(operand.prelude),
            .expression = context.target().append_expression({
                .value = TargetCallExpr {
                    ...
                    .template_arguments = {lower_type(context, source.type)},
...
auto lower_expression(
    ...
    if (expression.op == HIRBinaryExpr::Operator::LogicalOr
        ...
        const auto result = name_expression(context, TargetName {result_name});
        auto left = lower_expression(context, expression.left, control);
        if (is_foreign_type(context, context.semantic().expression(expression.left).type)) {
        ...
        if (is_foreign_type(context, context.semantic().expression(expression.right).type)) {
        ...
        auto evaluate_right = result;
        if (expression.op == HIRBinaryExpr::Operator::LogicalOr) {
            evaluate_right = context.target().append_expression({
                .value =
                    TargetPrefixExpr {.op = TargetPrefixOperator::LogicalNot, .operand_id = result},
            });
        }
        prelude.push_back(context.target().append_lowering_statement(
            TargetIfStmt {
    ...
    if (source.constant.has_value()) {
        ...
        if (is_integer_type(context, context.semantic().expression(expression.left).type)) {
    ...
    const auto left_foreign =
        is_foreign_type(context, context.semantic().expression(expression.left).type);
    const auto right_foreign =
        is_foreign_type(context, context.semantic().expression(expression.right).type);
    if (lowered_left.prelude.empty()
        && lowered_right.prelude.empty()
        && TargetEvaluationSequencer::effects_commute(
            context.semantic().expression_facts(expression.left).evaluation_effect,
            context.semantic().expression_facts(expression.right).evaluation_effect
        )) {
    ...
    auto left = TargetEvaluationSequencer::materialize(
        ...
        left_foreign ? MaterializationKind::Preserve
                     : TargetEvaluationSequencer::read_materialization(
                           context,
                           context.semantic().expression(expression.left).type
                       ),
...
auto lower_expression(
    ...
    operand.expression = context.target().append_expression({
        .value = TargetStaticCastExpr {
            .type = lower_type(context, context.semantic().expression(id).type),
