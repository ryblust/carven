module carven:backend.lowering.declarations.structure.impl;
...
auto lower_structure_declaration(TargetModuleLowerer& context, StructID structure_id) noexcept
    ...
    if (structure.supports_equality) {
        const auto structure_type =
            lower_type(context, *context.semantic().symbol(structure.symbol).type);
        members.push_back(
