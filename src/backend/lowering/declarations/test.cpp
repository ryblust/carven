module carven:backend.lowering.declarations.test.impl;
...
auto lower_test_declaration(TargetModuleLowerer& context, TestID test_id) noexcept
    ...
    const auto& source_body = context.semantic().body(test.body);
    ...
    auto callable_lowerer =
        context.callable(source_body.scope, context.semantic().block(source_body.root).scope);
    ...
    const auto module_name = context.target().append_expression({
        .value = TargetLiteralExpr {
            .value = TargetStringLiteral {
                .bytes = std::string(context.semantic()
                                         .provenance()
    ...
    const auto case_name = context.target().append_expression({
        .value = TargetLiteralExpr {
            .value = TargetStringLiteral {
                .bytes = std::string(context.semantic().provenance().spelling(test.name)),
                .kind = TargetStringLiteralKind::String,
            },
        },
    });
