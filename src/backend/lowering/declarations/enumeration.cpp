module carven:backend.lowering.declarations.enumeration.impl;
...
auto lower_enumeration_declaration(TargetModuleLowerer& context, EnumID enumeration_id) noexcept
    ...
    if (enumeration.profile == HIREnumProfile::Numeric) {
        ...
        for (const auto enum_case_id : enumeration.cases) {
            const auto& enum_case = context.semantic().enum_case(enum_case_id);
            ...
            const auto* constant = std::get_if<HIRNumericEnumConstant>(
                &context.semantic().constant(*enum_case.constant).value
            );
            if (constant == nullptr) {
    ...
    for (const auto member_id : enumeration.cases) {
        const auto& member = context.semantic().enum_case(member_id);
    ...
    const auto enum_type = lower_type(context, *context.semantic().symbol(enumeration.symbol).type);
    const auto equality = enumeration.supports_equality;
        ...
        public_members.push_back(
            TargetMemberFunctionDecl {
                ...
                .parameters =
                    {
                        {.name = std::nullopt,
                         .type = reference_type(context, enum_type, true),
                         .maybe_unused = false},
                        {.name = std::nullopt,
                         .type = reference_type(context, enum_type, true),
                         .maybe_unused = false},
    ...
    {
        auto alternatives = std::vector<TargetTemplateArgument> {};
        for (auto case_index = 0uz; case_index < members.size(); ++case_index) {
            ...
            alternatives.push_back(named_type(context, TargetName {record_name}));
        }
        const auto variant_type = context.target().intern_type({
            .value =
                TargetIntrinsicType {
                    .symbol = TargetSymbol::StdVariant,
                    .arguments = std::move(alternatives),
                },
            .const_qualified = false,
        });
        private_members.push_back(
    ...
    for (auto case_index = 0uz; case_index < members.size(); ++case_index) {
        ...
        const auto storage = name_expression(context, TargetName {storage_name});
        const auto condition = context.target().append_expression({
            .value = TargetCallExpr {
                ...
                .template_arguments = {named_type(context, TargetName {record_name})},
                .arguments = {storage},
            },
        });
        const auto returned =
