module carven:backend.lowering.declarations.impl;
...
auto lower_declaration(
    ...
    const auto origin = std::visit(
        Overloaded {
            [&](FunctionID id) noexcept { return context.semantic().function(id).origin; },
            [&](StructID id) noexcept { return context.semantic().structure(id).origin; },
            [&](EnumID id) noexcept { return context.semantic().enumeration(id).origin; },
        },
        declaration
    );
