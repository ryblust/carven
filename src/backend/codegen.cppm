...
constexpr auto generate_stmt(const Stmt& stmt, std::string_view source) noexcept -> std::string;
constexpr auto generate_expr(const Expr& expr, std::string_view source) noexcept -> std::string;
            ...
            if (s.init != nullptr) {
                std::visit(Overloaded {
                    [&](const VarDecl& d) {
                        ...
                        if (r.ends_with(";\n")) r.resize(r.size() - 2);  // strip trailing ;
                    },
                    [&](const ExprStmt& es) {
...
constexpr auto generate_expr(const Expr& expr, std::string_view source) noexcept -> std::string {
    return std::visit(Overloaded {
        ...
        [&](const IfExpr& e) -> std::string {
            // Expression-position if: check if branches are single-expr blocks
            const auto then_single = e.then_branch.statements.size() == 1;
            const auto else_single = e.else_branch.has_value() && e.else_branch->statements.size() == 1;

            if (then_single && else_single) {
                // Single-expr branches → ternary
                auto r = generate_expr(*e.condition, source).append(" ? ");
                const auto& then_s = std::get<ExprStmt>(*e.then_branch.statements[0]);
                r.append(generate_expr(*then_s.expr, source)).append(" : ");
                const auto& else_s = std::get<ExprStmt>(*e.else_branch->statements[0]);
                return r.append(generate_expr(*else_s.expr, source));
            }

            // Fallback: statement-style if
