...
race、synchronization relation，以及这些保证在 `#[cpp]` 或 C++ consumer boundary 何处停止。
...
- Write access 是 non-owning 且 **nonexclusive**：同一 mutable owner 可以在一次 call 中传给多个
  Write parameters；
- `#[cpp]` body 是 opaque C++ boundary；越过 Carven 已建模边界的 C++ control transfer、
  mutation、lifetime、concurrency 与 undefined-behavior obligation 归 boundary author；
...
synchronization，或把责任留在显式 `#[cpp]` boundary。
...
- [Threading](threading.md) 依赖本文的 data-race、value-capability 与 happens-before contract，
  但本文也可以由 non-async thread、channel、shared owner 或 interop use case 激活。
...
- 让 `#[cpp]` 与 C++ consumer boundary 成为 memory-model reasoning 的显式边界；
- 要求未来 feature 以 source semantics、compiler facts、lowering、interop、diagnostics、
  tests 与永久文档组成完整垂直切片。
...
- **Constraints:** 只描述当前事实，保持 Write 与 `#[cpp]` 责任边界，不选择未来模型。
- **Options:** 当前 proposal 建议采用等价英文陈述：Carven 当前不定义跨线程 memory model，不提供
  ...
  target data-race validity 归 integration author；`#[cpp]` 内并发行为继续服从既有 boundary
  responsibility。该措辞尚未接受。
...
- **Closure condition:** 用一个 concrete shared-state example 定义 ordinary-source guarantee、
  aliasing proof boundary 与准确的 `#[cpp]`/consumer handoff。
...
guarantee、diagnostics、SemanticProgram facts、target/runtime、`#[cpp]` 与 consumer adapter、tests 与永久
文档同时闭合。实现必须由具体 movement、sharing 与 synchronization operation 驱动，而不是预造
...
- 不与现有 Write/access、`#[cpp]` responsibility、callable-view lifetime 或 string-view lifetime 冲突；
...
generated C++ compile/link/run、interop boundary 与相应 resource/cost evidence。
