...
using semantic_analysis_test::find_code;
...
TEST_CASE("Semantic availability: completed values release call accesses") {
    ...
    const auto direct =
        analyze_errors(prelude + "fn invalid() { let x = 2; let result = pair(x, &&x); }");
    CHECK(contains_code(direct, DiagnosticCode::AccessOperationConflict));
}

TEST_CASE("Semantic availability: direct self transfer cannot restore its source") {
    for (const auto* expression : {"&&x", "(&&x)"}) {
        const auto diagnostics =
            analyze_errors(std::string("fn invalid() { var x = 2; x = ") + expression + "; }");
