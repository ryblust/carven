...
TEST_CASE("Compiler diagnostics: external delegation retains Carven access rules") {
    constexpr auto cases = std::to_array<ErrorExpectation>({
        ...
        {.name = "global keyword type",
         ...
         .code = "CV-CPP-IDENTIFIER",
         .primary_text = "class"},
        {.name = "global Write cannot mutate let",
         .source = "fn f() { let x = 1; ::change(&x); }",
         .code = "CV-ACCESS-IMMUTABLE",
         .primary_text = "x"},
        {.name = "global call cannot read taken owner",
         .source = "fn f() { let x = 1; ::consume(&&x); ::consume(x); }",
         .code = "CV-ACCESS-UNAVAILABLE",
         .primary_text = "x"},
        {.name = "global result is not a constant",
         .source = "fn f() { const x = ::value(); }",
